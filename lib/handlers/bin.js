...
module.exports = Observable.extend({
  ...
  getDefault: function(req, res, next) {
    if ((req.subdomain && custom[req.subdomain]) || custom.default) {
      console.info('getCustom')
      return this.getCustom(req, res, next);
    }
    console.info('renderFiles')
  ...
  renderFiles: function(req, res, next) {
    ...
    var files = this.defaultFiles(template)
    console.info(`files is ${JSON.stringify(files)}`)
    this.loadFiles(files, function(err, results) {
      console.info(`result is ${JSON.stringify(results)}`)
      if (!err) {
        if (next) {
          req.bin = results;
          console.info('next')
  ...
  defaultFiles: function(template) {
    let typeFlag = `-${template}`;
    if(!template || template == 'react18') {
      typeFlag = ''
    }
    return ['html', 'css', 'js'].map(function(ext) {
      console.info(`get default', ${template} ${typeFlag} ${ext}`)
      if(ext == 'html') {
        return `default${typeFlag}.` + ext;
      }
      return 'default.' + ext;
    });
  },
